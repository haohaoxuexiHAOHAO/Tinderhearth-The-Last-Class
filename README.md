...
**门禁只调这一条。** 它把构建 → 测试 → 导出 → 跑产物四步串起来，逐步日志落到
`logs/verify/<时间戳>/`（不入库），标准输出每步只有一行，同目录写一份带起止时间戳的
`summary.md`。失败即停 —— 后一步依赖前一步的产物，硬跑下去只会产出误导性的失败。
...
两个附带用法：`--upto build|test|export` 只跑到某步（前置步骤一定跟着跑，所以跑不出旧
