...
/// 规则。素材那一半原先有逐像素守卫，随 `ADR-0009` 删除后改为人工标准（画得不对实机看得出来）；
/// 界面颜色这一半的执行体**仍然在、而且更硬** —— 一个 50% 不透明的遮罩画上去，屏幕上出现的就是
/// 插值像素，与半透明素材同一后果，而这个类型里根本没有那个字段，所以没有人能顺手加。
/// 类型缺字段比守卫可靠：它在编译期就拦住，不依赖有人去跑检查。
///
...
/// 对比工程（设计仓 `tools/font-preview/main.gd`）里用过的占位配色。理由是连续性 —— 判定
/// 「12px 可读」时看的就是这个背景上的这个字色，换一套等于让那次判断作废。
///
/// 四条资源色按 [像素绘制原则 §4]「明度先于色相」挑：四条的明度各不相同，
