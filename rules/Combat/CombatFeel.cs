...
///
/// **不进 `design/数值模型.md`。** 那份持有的是能进不等式判据的平衡量。手感帧数（前后摇、无敌窗、
/// 顿帧、击退量）一条公式都不进 —— 正典明确把它们排出数值模型，
...
public static class CombatFeel
{
    ...
    /// <summary>重攻击连段的段数。**取 1：重击是单招、不连段**，后续可能再加连段。</summary>
    ...
    /// <summary>轻击击退距离，世界像素。**实机定为 0**：轻击有击退会把敌人推出连段射程、只能平 A 一下就够不到，归 0 让轻击连段留在射程里。重击照旧击退。</summary>
    ...
    ///
    /// 取 8（±8%）：能听出每下不同，又不会把打击音抖成音阶。**实机听过，占位阶段可接受**；
    /// 采样本身后续会替换，届时这个数大概要跟着重听一遍。
    ///
    /// **正式音效的采样率、总线与混音归
    ...
    /// <summary>命中当帧的极短白闪时长，**真实帧**（不受顿帧冻结）；`GP-20`，**实机确认为 2**。</summary>
    /// <remarks>
    ...
    /// 就整个看不见，而这一下正是要让眼睛看到的。**「几帧才不晃眼」只能实机看**，作者已在训练房
    /// 确认这一版可以，所以它不再是未校准初值。
    ///
    /// **但它没有被单独隔离验证过**：确认时场上还没有打击音，顿帧与震屏也还没按「爆点」协同过。
    /// 四件事叠在同一帧的观感会互相影响，届时若要重调走 `GP-6` 并写清新依据 —— 这条确认只覆盖
    /// 「白闪 ＋ 已有的顿帧/震屏/火花」这个组合。
    /// </remarks>
    ...
    /// <remarks>
    /// **影子宽度跟着当前那一帧的本体宽度走** —— 攻击与闪避时影子一动不动，实机看上去很奇怪。
    /// 取 90：站着时本体 19px，影子 17px，比脚略窄、不从脚边露出来；
    /// 拳伸到最远时本体 29px，影子跟着拉长。
    ///
    /// **逐帧取，不按动作取最大值。** 后者（同一动作内影子恒定，为躲开待机呼吸的 1px 抖动）
    /// **被实测否掉**：轻击整段的影子都是伸出后的 26px，而前摇帧身体只有 19px，
    ...
    //
    // **按段分开取值。** 轻击三段各有独立动画（`light`／`light2`／`light3`），
    // 三段打出去的拳脚伸到的距离与高度都不一样 —— 尤其第 3 段是踢腿，伸得比前两段的直拳都远、
    // 且更低。若三段共用一个框，画面上那一脚踢到脚边、
    // 判定框却停在拳的位置，玩家读到的是「这一下明明够到了却没打中」，且这件事**不报错**。重击
    // 仍是单招、自成一段。
    //
    // 下面每个数不是估的，是从精灵表量出来的：取「伸出静止起手姿之外的那部分」（也就是打出去的
    // 那只拳脚）的包围盒，按 Active 窗口（<c>PlayerActor.AttackActiveFirstFrame</c> 与
    // <c>AttackActiveSpan</c>）**逐段**取极值 —— 宽＝Active 两帧的最大右伸，高＝Active 两帧行区间
    // 的并，中心＝脚底之上到那个区间中点的高度。
    //
    // **原先有守卫把这三处（素材／Active 窗口／这里的常量）绑在一起逐条比对，那条链已随
    // `ADR-0009` 删除。** 于是三处任意一处改了不会再被当场拦下 —— 改素材或改 Active 窗口时，
    // 请连带在 Godot 里开训练房的判定框叠层（按 <c>V</c>）看一眼框还贴不贴那只拳脚。
    //
    // **仍是未校准初值，归 `GP-6`。** 量出来的是「跟画面对齐」，不是「手感对」—— 尤其轻击框只有
    // 三四像素高（作者画的直拳本身就这么高），实机若发现容易打空，那是 `GP-6` 要调的宽容量，不是
    // 回头改这条对齐口径。中心离脚底的高度也按段：直拳落在脚底上方约 17px，踢腿更低（实测 15.5、
    // 取整 16）—— 共用一个 18 会让踢腿的判定框浮在腿上方，那正是「画面与判定对不上」。
    ...
    /// <summary>实体阻挡的纵深阈值，世界像素；`GP-17` 未校准初值，归 `GP-6`。</summary>
    /// <remarks>
    /// **与 <see cref="HitDepthToleranceWorldPx"/> 是两个数，刻意不共用常量**（设计仓 `issue-GP-17`
    /// 与 `ARCHITECTURE.md` 都点名）：那个是「打得着」的宽容量，这个是「占同一格」的物理量。两者
    /// 此刻都等于 8 是**巧合，不是等式** —— 它们会被 `GP-6` 各自单独改，共用一个常量会让改一个静默
    /// 改掉另一个。
    ///
    /// 取 8 是算出来的，不是凭手感给的。依据是正典那笔几何账里的两个可读性结论
    /// （`canon/gameplay/战斗与关卡.md` 的「48px 是怎么算出来的」）：**纵深差 8px 以下基本重叠**，
    /// **相邻两排差 16px 时 30px 高的精灵仍读得出谁在前**。于是把阈值取成半排：
    ///
    /// - 看上去重叠的两具身体（差 ≤ 8px）互相挡住 —— 玩家看到的是「贴在一起」，物理就该同意。
    ///   反面正是实机撞到过的那件事：看着该挡，人却穿过去了。
    /// - 隔一排（差 16px ＝ <see cref="DepthBand.RowSpacingWorldPx"/>）不挡，且离阈值有整整一倍
    ///   余量，不会在浮点末位上摇摆。**这一头比另一头更要紧**：不该挡的挡了，玩家看到的是两个明显
    ///   不在一排的东西卡在一起，他读不出原因。
    /// - 纵深连续、不是轨道，所以同一排上两个人各偏半排是常态，最坏情况纵深差正好一排的一半 ——
    ///   阈值取半排刚好把「都算在这一排」覆盖满。
    ///
    /// **另一条看起来更「物理」的推法被否掉了**：拿影子的纵深厚度（<see cref="ShadowHeightWorldPx"/>
    /// ＝6）当本体占地。它本身是未校准的审美初值（「约为宽度的三分之一」「明显小于一排、免得相邻
    /// 两排的影子连成一片」），拿它当阻挡阈值会让**改影子长相就改碰撞行为** —— 那是不该有的耦合。
    ///
    /// **一条不变量由单测钉着：本阈值 ≤ <see cref="HitDepthToleranceWorldPx"/>。** 反过来会出现
    /// 「被挡住却打不着」—— 玩家贴着一个够不到的东西，他读不出原因。两个数都能被 `GP-6` 改，改到让
    /// 这条关系不成立就是改坏了。
    ///
    /// **阈值是一个数、不按物件分。** 一块巨石在纵深上本该比角色占得多，但按物件给纵深厚度要先有
    /// 真实的关卡物件，先给一个数；够不够只能实机看，归 `GP-6`。
    /// </remarks>
